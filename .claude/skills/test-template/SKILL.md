...
- **Sandbox**: Bash localhost calls require `dangerouslyDisableSandbox: true`. Use it for both `lsof` checks and the `node ${TMPDIR:-/tmp}/devhub-skills/test-template/test-template.mjs` invocation.
