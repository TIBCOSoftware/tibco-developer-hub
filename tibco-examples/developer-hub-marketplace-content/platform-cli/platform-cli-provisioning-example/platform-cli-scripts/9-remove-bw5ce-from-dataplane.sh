...
echo "---------------  Removing BW5CE from a Dataplane ------------------------";
...
#USAGE
#  $ tibcop tplatform:delete-capability-instance -n BW5CE|TIBCOHUB|FLOGO|EMS|PULSAR [--no-warnings] [--config <value>] [--profile <value> | --cpurl <value> | --token <value>] [-i <value> | -n <value>] [-d]
#
#FLAGS
#  -d, --debug
...
#                                <options: BW5CE|TIBCOHUB|FLOGO|EMS|PULSAR>
...
CAPABILITY_ID=$(tibcop tplatform:list-capability-instances --dataplane-id="$DATAPLANE_ID" --json --profile "$profile" | jq --arg NAME "BW5CE" '.[] | select(.name == $NAME) | .id' | sed 's/"//g')
...
echo "Runnning command:|tibcop tplatform:delete-capability-instance --id BW5CE --dataplane-name=\"$DP_NAME\" --debug --profile \"$profile\"|"
tibcop tplatform:delete-capability-instance --id BW5CE --dataplane-name="$DP_NAME" --debug --profile "$profile"

echo "Capability instances in the dataplane (After removing BW5CE):"
tibcop tplatform:list-capability-instances --dataplane-name="$DP_NAME" --profile "$profile"
