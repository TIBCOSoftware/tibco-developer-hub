...
echo "---------------  Removing BWCE from a Dataplane ------------------------";
...
echo "Runnning command:|tibcop tplatform:delete-capability-instance --id BWCE --dataplane-name=\"$DP_NAME\" --debug --profile \"$profile\"|"
tibcop tplatform:delete-capability-instance --id BWCE --dataplane-name="$DP_NAME" --debug --profile "$profile"

echo "Capability instances in the dataplane (After removing BWCE):"
tibcop tplatform:list-capability-instances --dataplane-name="$DP_NAME" --profile "$profile"
