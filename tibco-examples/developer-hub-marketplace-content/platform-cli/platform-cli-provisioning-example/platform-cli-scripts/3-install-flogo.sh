...
# Path Prefix for Flogo
PATH_PREFIX="${3:-/flogo/$version}"
# FQDN for the Flogo Capability
fqdn="${4:-$version.flogo.developer-experience.dataplanes.pro}"

RESOURCE_INSTANCE_NAME_STORAGE="${5:-StorageResourceFlogo_$version}"
RESOURCE_INSTANCE_NAME_INGRESS="${6:-IngressResourceFlogo_$version}"
...
echo "---------- Installing a The Flogo capability  -----------------------";
...
echo tibcop tplatform:provision-capability --dataplane-name "$DP_NAME" --capability FLOGO  --storage-resource-instance-id "$STORAGE_RESOURCE_ID" --ingress-resource-instance-id "$INGRESS_RESOURCE_ID" --path-prefix "$PATH_PREFIX" --fluentbit-sidecar-enabled --profile "$profile" --debug
tibcop tplatform:provision-capability --dataplane-name "$DP_NAME" --capability FLOGO  --storage-resource-instance-id "$STORAGE_RESOURCE_ID" --ingress-resource-instance-id "$INGRESS_RESOURCE_ID" --path-prefix "$PATH_PREFIX" --fluentbit-sidecar-enabled --profile "$profile" --debug


# Step 4] Wait for the Flogo Capability to be up
...
# The name is flogo here
CAPABILITY_ID=$(tibcop tplatform:list-capability-instances --dataplane-id="$DATAPLANE_ID" --json --profile "$profile" | jq --arg NAME "FLOGO" '.[] | select(.name == $NAME) | .id' | sed 's/"//g')
echo "Capability ID: $CAPABILITY_ID"

echo "Waiting for the Flogo Capability to be up..."
counter=0
max_retry=120
# tibcop tplatform:validate --type capability --dataplane-id="d15ap9oocqe9imoh33ig" --id "d15ar58ocqe9imoh33m0" -s "green" --profile "PM CP"
...
do
   ...
   echo "Trying to see if the Flogo Capability is up. Try #$counter"
   ((counter++))
done
...
# Calculate minutes
echo "-------------------------------------------------------------------------";
echo "-- It took $(convertsecs $runtime_e2e) to create the Flogo Capability..."
echo "-------------------------------------------------------------------------";
echo "-------------------------------------------------------------------------"
echo "----        Dataplane Name: $DP_NAME"
echo "----           Path Prefix: $PATH_PREFIX"
echo "-------------------------------------------------------------------------"
echo "-- You can find the Flogo API's here: https://$fqdn$PATH_PREFIX/public/dp/docs "
