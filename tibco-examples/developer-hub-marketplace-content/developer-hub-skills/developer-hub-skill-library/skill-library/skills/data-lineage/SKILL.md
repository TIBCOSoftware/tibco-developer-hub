...
Complete worked examples over a `sap-integration-hub` catalog — `lineage-index.md` and the four
reports it links (`lineage-materialnumber.md`, `lineage-inventory-update-msg.md`,
`lineage-quantity.md`, `lineage-planned-gi-date.md`) plus `lineage-data-snapshot.md` — are
published with the **Developer Hub Skills Library** marketplace entry documentation. Read them for
the expected depth, tier discipline, and diagram style before generating a new one.
